...
gantt.directive('gantt', ['Gantt', 'dateFunctions', 'mouseOffset', 'debounce', 'keepScrollPos', function (Gantt, df, mouseOffset, debounce, keepScrollPos) {
    return {
        ...
        controller: ['$scope', '$element', function ($scope, $element) {
            ...
            if ($scope.columnWidth === undefined) $scope.columnWidth = 10;
            ...
            $scope.gantt.expandDefaultDateRange($scope.$parent.fromDate, $scope.$parent.toDate);
...
;gantt.factory('Column', [ 'dateFunctions', function (df) {
    ...
    var QuarterColumn = function (date, left, width, subScale) {
        var column = new Column(date, left, width, subScale);
        column.daysInQuarter = df.getDaysInQuarter(column.date);
        column.quarter = df.getQuarter(date);
        ...
        column.getPositionByDate = function (date) {
            return calcPbyD(column, date, column.daysInQuarter, date.getDate(), getQuarter(date), getQuarter(column.date));
        };
    ...
    var generateQuarterHeader = function (columns) {
        ...
        for (var i = 0, l = columns.length; i < l; i++) {
            var col = columns[i];
            if (i === 0 || df.getQuarter(columns[i - 1].date) !== df.getQuarter(col.date)) {
                header = new Column.Quarter(df.clone(col.date), col.left, col.width);
                generatedHeaders.push(header);
            } else {
                header.width += col.width;
            }
    ...
    return {
        ...
        addQuarters: function(date, val, clone) {
            var res = clone === true ? this.clone(date) : date;
            res.setDate(1);
            res.setMonth(res.getMonth() + val * 3);
            return res;
        },
        ...
        getDaysInQuarter: function (date) {
            var q, days;
            q = this.getQuarter(date);
            days = new Date(date.getYear(), 1 + (q - 1) * 3, 0).getDate() + new Date(date.getYear(), 2 + (q - 1) * 3, 0).getDate() + new Date(date.getYear(), 3 + (q - 1) * 3, 0).getDate();
            return days;
        },
