gantt.factory('ColumnGenerator', [ 'Column', 'dateFunctions', function (Column, df) {
    ...
    var QuarterColumnGenerator = function (columnWidth, columnSubScale) {
        this.generate = function (from, to) {
            var excludeTo = to.getDate() === 1 && df.isTimeZero(to);
            from = df.setToFirstDayOfQuarter(df.setTimeZero(from, true), false);
            to = df.setToFirstDayOfQuarter(df.setTimeZero(to, true), false);
            ...
            while (excludeTo && to - date > 0 || !excludeTo && to - date >= 0) {
                generatedCols.push(new Column.Quarter(df.clone(date), left, columnWidth, columnSubScale));
                left += columnWidth;

                date = df.addQuarters(date, 1);
            }
