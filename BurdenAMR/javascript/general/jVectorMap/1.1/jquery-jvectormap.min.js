...
(function (e) {
    ...
    e.fn.vectorMap = function (e) {
        var labels = $(".jvectormap-label");
        if (labels.length > 1) labels.first().remove();
...
jvm.$ = jQuery, jvm.AbstractElement = function (e, t) {
    ...
    bindZoomButtons: function () {
        var e = this;
        if (this.params && this.params["options"] && this.params.options["ToggleDownloadLink"]) {
            jvm.$("<div/>").addClass("jvectormap-download").text("Download Map").appendTo(this.container);
        }
        jvm.$("<div/>").addClass("jvectormap-zoomin").text("+").attr('title', 'Zoom In').appendTo(this.container), jvm.$("<div/>").addClass("jvectormap-zoomout").attr('title', 'Zoom Out').html("&#x2212;").appendTo(this.container), this.container.find(".jvectormap-zoomin").click(function () {
    ...
    getRegionCentroid: function (region) {
        if (this.regions[region] == undefined) return [null, null];
        if (region == 'US') {
            return [134.387917, 164.454357];
        } else if (region == 'CA') {
            return [130.22053680419921, 99.642506];
        } else if (region == 'FR') {
            return [419.90872, 156.538589];
        } else if (region == 'RU') {
            var map = this,
                element = map.regions[region].element,
                bbox = element.getBBox(),
                xcoord = (bbox.x + (bbox.width * 0.33)) - 50,
                ycoord = (bbox.y + (bbox.height * 0.33)) + 45;
            ...
            var map = this,
                element = map.regions[region].element,
                bbox = element.getBBox(),
                xcoord = (bbox.x + (bbox.width * 0.33)) - 10,
                ycoord = (bbox.y + (bbox.height * 0.33)) + 25;
            ...
            var map = this,
                element = map.regions[region].element,
                bbox = element.getBBox(),
                xcoord = (bbox.x + (bbox.width * 0.33)) - 5,
                ycoord = (bbox.y + (bbox.height * 0.33)) + 13;
            ...
            var map = this,
                ...
                bbox = element.getBBox(),
                xcoord = (bbox.x + (bbox.width * 0.33)) - 10,
                ycoord = (bbox.y + (bbox.height * 0.33)) + 15;
            ...
            var map = this,
                ...
                bbox = element.getBBox(),
                xcoord = (bbox.x + (bbox.width * 0.33)) - 10,
                ycoord = (bbox.y + (bbox.height * 0.33)) + 5;
            ...
            var map = this,
                element = map.regions[region].element,
                bbox = element.getBBox(),
                xcoord = (bbox.x + (bbox.width * 0.33)),
                ycoord = (bbox.y + (bbox.height * 0.33));
            ...
            var map = this,
                ...
                bbox = element.getBBox(),
                xcoord = (bbox.x + (bbox.width * 0.33)),
                ycoord = (bbox.y + (bbox.height * 0.43));

            return [xcoord, ycoord];
        }
    },
    removeAllTextFromMap: function () {
        var group = this.container.children(0).children(0)[0];
        $(group).find("text").remove();
        $(group).find("rect").remove();
    },
    removeAllTextFromRegion: function (region) {
        $("#region_text_" + region).remove();
        $("#region_text_bg_" + region).remove();
    },
    addTextToRegion: function (text, region, color, rotation, css, withBG, fontSize) {
        this.removeAllTextFromRegion(region);
        var group = this.container.children(0).children(0)[0];
        fontSize = fontSize == null || fontSize == undefined ? 5 : fontSize;
        var coords = { x: this.getRegionCentroid(region)[0], y: this.getRegionCentroid(region)[1] };

        if (coords.x == null || coords.y == null) return;

        if (color == null || color == undefined) color = 'white';

        if (withBG != undefined && withBG != null && withBG == true) {
            var bgColor = tinycolor.mostReadableAllColors(color).hex;
            var BG = document.createElementNS("http://www.w3.org/2000/svg", "rect");
            BG.setAttributeNS(null, "x", coords.x - 1);
            BG.setAttributeNS(null, "y", coords.y - fontSize - 2);
            BG.setAttributeNS(null, "width", text.length * (fontSize / 1.7));
            BG.setAttributeNS(null, "height", fontSize + 6);
            BG.setAttributeNS(null, "rx", 1);
            BG.setAttributeNS(null, "ry", 1);
            if (rotation != null)
                BG.setAttributeNS(null, "transform", "rotate(" + rotation + "," + coords.x + "," + coords.y + ")");
            BG.style.cssText = "fill:" + bgColor + ";";
            BG.setAttributeNS(null, "id", "region_text_bg_" + region);
            group.appendChild(BG);
        }

        var T = document.createElementNS("http://www.w3.org/2000/svg", "text");
        T.setAttributeNS(null, "x", coords.x);
        T.setAttributeNS(null, "y", coords.y);
        T.setAttributeNS(null, "id", "region_text_" + region);
        if (rotation != null)
            T.setAttributeNS(null, "transform", "rotate(" + rotation + "," + coords.x + "," + coords.y + ")");

        if (css == null || css == undefined)
            T.style.cssText = "text-shadow:2px 2px 2px; font-family: 'DIN Regular'; font-size:" + fontSize + "px;";
        else
            T.style.cssText = css + " font-size: " + fontSize + ";";

        if (color != null && color != undefined && color.length > 0) T.style.cssText += " fill:" + color + " !important;";
        ...
        if (text != null && text != undefined) {
            if (typeof text === 'string') {
                ...
                for (var i = 0; i < text.length; i++) {
                    T.innerHTML += "<tspan x=" + coords.x + " y=" + (coords.y + (i * fontSize)) + ">" + text[i] + "</tspan>";
                }
            }
        }
        group.appendChild(T);
