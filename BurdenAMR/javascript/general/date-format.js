...
Date.parseFormatted = function (value, format)
{
	...
	var parts		= [];
	parts['d']		= '([0-9][0-9]?)';
	parts['dd']		= '([0-9]?[0-9])';  //20180828 - avathsalan
//	parts['ddd']	= NOT SUPPORTED;
//	parts['dddd']	= NOT SUPPORTED;
	parts['M']		= '([0-9][0-9]?)';
	parts['MM']		= '([0-9][0-9])';
	parts['MMM']    = '([a-zA-Z]+?)'; //20180827 - avathsalan
//	parts['MMMM']	= NOT SUPPORTED;
	...
	if(matches === undefined || matches === null || matches.length !== outputs.length)
	{
		return undefined;
	}
		...
		if(outputs[i] !== '')
		{
			switch(outputs[i])
			{
			    ...
			    case "MMM":
			        ...
			        var _shortMonth = (matches[i] + "").toLowerCase();

			        for (var m = 0; m < Date.formatLogic.i18n.shortMonthNames.length; m++) {
			            var _month = Date.formatLogic.i18n.shortMonthNames[m];

			            _month = _month.toLowerCase();

			            if (_month == _shortMonth) {
			                output.setMonth(m);
			                break;
			            }
			        }
